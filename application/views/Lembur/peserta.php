<?php echo form_open_multipart('Lembur/insert_peserta'); ?>
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th>Nama Peserta</th>
              <th>Unit</th>
              <th>Jam Mulai Presensi</th>
              <th>Jam Selesai Presensi</th>
              <th>Opsi</th>
            </tr>
          </thead>
          <tbody id="tbl_peserta">
            <?php foreach ($peserta as $value):
              $data_p = $this->ModelPegawai->edit($value->pegawai_uuid)->row_array()
            ?>
              <tr id="<?php echo $value->pegawai_uuid ?>">
                <td><input type='text' name='uuid[]' class='uuid' value='<?php echo $value->pegawai_uuid ?>' hidden><?php echo $data_p["NIP"] ?></td>
                <td><?php echo $data_p["nama_pegawai"] ?></td>
                <td><?php echo $data_p["unit"] ?></td>
                <td><input type='time' name='jam_mulai[]' class='form-control' value='<?php echo isset($value->jam_mulai) ? $value->jam_mulai : "" ?>' required></td>
                <td><input type='time' name='jam_selesai[]' class='form-control' value='<?php echo isset($value->jam_selesai) ? $value->jam_selesai : "" ?>' required></td>
                <td><button type='button' onclick='hapus("<?php echo $value->pegawai_uuid ?>")' class='btn btn-floating btn-danger'><i class='fas fa-user-slash'></i></button></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
          <tfoot>
            <th colspan="5" style="text-align:right;">TOTAL PESERTA:</th>
...
<script type="text/javascript">
  ...
  <?php foreach ($peserta as $value) { ?>
    pilih_kode.push("<?php echo $value->pegawai_uuid ?>");
  <?php } ?>
  ...
  function pilih(kode, nip, unit) {
    ...
    if (pilih_kode.indexOf(kode) < 0) {
      ...
      var tbody = "<tr id='" + kode + "'>" +
        "<td><input type='text' name='uuid[]' class='uuid' value='" + kode + "' hidden>" + nip + "</td>" +
        "<td>" + nama + "</td>" +
        "<td>" + unit + "</td>" +
        ...
        "<td><button type='button' onclick='hapus(\"" + kode + "\")' class='btn btn-floating btn-danger'><i class='fas fa-user-slash'></i></button></td>" +
        "</tr>";
