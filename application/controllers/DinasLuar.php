...
class DinasLuar extends CI_Controller
{
  ...
  function index()
  {
    ...
    $data = array(
      ...
      'body'          => 'DinasLuar/list',
      'DinasLuar'     => $this->ModelDinasLuar->getAll($tgl_mulai, $tgl_akhir)->result(),
      'tgl_mulai'     => $tgl_mulai,
      'tgl_akhir'     => $tgl_akhir
    );
  ...
  {
    $keterangan = $this->input->post("keterangan_approval");
    $approval_by = $this->session->userdata('nama'); // Sesuaikan dengan session user Anda
    ...
    $this->db->where("iddinas_luar", $id);
    if ($this->db->update('dinas_luar', $data)) {
      $this->session->set_flashdata('notifJS', $this->core->NotifSuccess("Status Approval Berhasil Diubah"));
      redirect(base_url() . 'DinasLuar');
    } else {
      $this->session->set_flashdata('notifJS', $this->core->NotifError("Gagal Mengubah Status Approval"));
      redirect(base_url() . 'DinasLuar');
    }
  }
}
