<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Kegiatan extends CI_Controller{

  public function __construct()
  {
    parent::__construct();
    //Codeigniter : Write Less Do More
    $this->load->model("ModelKegiatan");
    $this->load->model('ModelAuth');
    $this->ModelAuth->verify_token();
  }

  function getKegiatan()
  {
    $uuid = $this->input->post("uuid");
    $kampus = $this->ModelKegiatan->get_kegiatan_terkini($uuid);
    if ($kampus->num_rows() > 0) {
      $res = array(
        'message' => "Success",
        'status' => 200
      );
    }else {
      $res = array(
        'message' => "Maaf Tidak Bisa Ambil Data, Mohon Cek Koneksi Anda",
        'status' => 500
      );
    }
    echo json_encode(array('data'=>$kampus->result(),'message'=>$res));
  }

  function absen_kegiatan()
  {
    $patch = "document/foto_kegiatan/";
    // echo $patch;
    $config['upload_path']          = "./".$patch;
    $config['allowed_types']        = '*';
    $config['max_size']             = 11240;
    $data = array();
    $kegiatan = $this->ModelKegiatan->cekKode($this->input->post("idkegiatan"))->row_array();
    if (strtotime('now') >= strtotime($kegiatan['tanggal'])) {
      $this->db->where("kegiatan_idkegiatan", $this->input->post("idkegiatan"));
      $this->db->where("pegawai_uuid", $this->input->post("id"));
      $this->db->where("LEFT(jam_presensi, 10) = ", date("Y-m-d"));
      if ($this->db->get("absen_kegiatan")->num_rows() < 1) {
        $this->load->library('upload', $config);
        if ($this->upload->do_upload('image')) {
          $data = array(
            'jam_presensi'         => date("Y-m-d H:i:s"),
            'kegiatan_idkegiatan'  => $this->input->post("idkegiatan"),
            'pegawai_uuid'         => $this->input->post("id"),
            'absen_latitude'       => $this->input->post("lat"),
            'absen_longtitude'     => $this->input->post("long"),
            'status_lokasi'        => $this->input->post("status_lokasi"),
            'foto'                 => $patch.$this->upload->data()['file_name']
          );
          if ($this->db->insert("absen_kegiatan", $data)) {
            // $id_insert = $this->db->insert_id();
            // $this->db->where("uuid", $this->input->post("id"));
            // $this->db->update("pegawai", array("idabsen"=>$id_insert,"status_absen"=>"1"));
            $res = array(
              'message' => "Berhasil",
              'status' => 200
            );
          }else {
            $res = array(
              'message' => "Gagal Menyimpan",
              'status' => 501
            );
          }
        }else {
          $res = array(
            'message' => "Gagal",
            'status' => 500
          );
        }
      }else {
        $res = array(
          'message' => "Anda Sudah Melakukan Presensi Kegiatan, Bisa dilihat di Laporan Kegiatan",
          'status' => 500
        );
      }

    }else {
      $tgl_ket = date("d-m-Y", strtotime($kegiatan['tanggal']));
      if ($kegiatan['tanggal'] != $kegiatan['tanggal_selesai']) {
        date("d-m-Y", strtotime($kegiatan['tanggal']))." s/d ".date("d-m-Y", strtotime($kegiatan['tanggal_selesai']));
      }
      $res = array(
        'message' => "Belum Waktunya Presensi, Kegiatan Tanggal ".$tgl_ket,
        'status' => 500
      );
    }

    echo json_encode(array('response'=>$data,'message'=>$res));
  }

  function cek_absen_kegiatan($idkegiatan, $uuid)
  {
    $kampus = $this->ModelKegiatan->get_cek($idkegiatan, $uuid, date("Y-m-d"));
    if ($kampus->num_rows() > 0) {
      $res = array(
        'message' => "Success",
        'status' => 200
      );
    }else {
      $res = array(
        'message' => "Maaf Tidak Bisa Ambil Data, Mohon Cek Koneksi Anda",
        'status' => 500
      );
    }
    echo json_encode(array('data'=>$kampus->result(),'message'=>$res));
  }

  function list_aproval()
  {
    $unit = $this->input->post("unit");
    $data = array();
    $riwayat = $this->ModelKegiatan->getKegiatanAproval($unit,"0");
    if ($riwayat->num_rows() > 0) {
      foreach ($riwayat->result() as $value) {
        $kegiatan = $this->ModelKegiatan->get_data($value->kegiatan_idkegiatan)->row_array();
        $ar = array(
          'idabsen_kegiatan'=> $value->idabsen_kegiatan,
          'nama_pegawai'    => $value->nama_pegawai,
          'NIP'             => $value->NIP,
          'absen_latitude'  => $value->absen_latitude,
          'absen_longtitude'=> $value->absen_longtitude,
          'foto'            => $value->foto,
          'jam_presensi'    => date("H:i:s", strtotime($value->jam_presensi)),
          'tgl_presensi'    => date("D, d M Y", strtotime($value->jam_presensi)),
          'status_aproval'  => $value->status_aproval,
          'kegiatan'        => $kegiatan
        );
        array_push($data, $ar);
      }
      $res = array(
        'message' => "Success",
        'status' => 200
      );
    }else {
      $res = array(
        'message' => "Maaf Tidak Bisa Ambil Data, Mohon Cek Koneksi Anda",
        'status' => 500
      );
    }
    echo json_encode(array('data'=>$data,'message'=>$res));
  }

  function approval_kegiatan()
  {
    $idabsensi = $this->input->post("idabsensi");
    $approval = $this->input->post("approval");
    $this->db->where("idabsen_kegiatan", $idabsensi);
    if ($this->db->update("absen_kegiatan", array('status_aproval' => $approval))) {
      $res = array(
        'message' => "Success",
        'status' => 200
      );
    }else {
      $res = array(
        'message' => "Maaf Tidak Bisa Ambil Data, Mohon Cek Koneksi Anda",
        'status' => 500
      );
    }
    echo json_encode(array('message'=>$res));
  }

}
